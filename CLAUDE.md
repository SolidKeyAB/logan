...
| `logan_save_report` | Save the current investigation as a self-contained **markdown DOC** in `.logan/reports/`: clear name + AIM + REASON + optional ticket, folding in pinned findings, recorded steps, and (opt-in) the root-cause verdict. A durable write-up to read/share/attach to a ticket |
