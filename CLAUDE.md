...
| `logan_diff_runs` | **Run-vs-run template diff**: "what does this (failing) run contain that a good run doesn't?" Folds the open log + a `reference` log (by path, opened on demand) into message templates and set-diffs them → `onlyInTarget` (new shapes — the headline), `onlyInReference` (vanished), `changed` (frequency shifted ≥`changeFactor`×). Each delta has counts, factor, severity, example `viewerLine`s. Semantic multi-log differential — beats a raw line diff on big logs, finer than `baseline_compare`. Point `reference` at the last-known-good run; investigate `onlyInTarget` first, then pin with `logan_report_finding` |
