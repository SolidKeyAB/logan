...
- **Agent save report — `logan_save_report`** (bundle the current investigation into one
  self-contained `.md` doc: clear name + AIM + REASON + optional ticket, folding in pinned
  findings, the recorded steps, and — opt-in — the native root-cause verdict). Agent surface:
  MCP `logan_save_report` + `/api/save-report` → pure `reportDoc.ts` builder → `ctx.saveReport`
  writes to the log's `.logan/reports/<slug>.report.md` (read-only fallback to
  `~/.logan/reports/<basename>/`). Human surface: the saved doc is announced in the **chat
  panel** with its clickable path, and is a plain markdown file the user opens (markdown
  handler), shares, or attaches to a ticket. **Written create-side exemption:** the report is
  the *agent's* work product — it captures the agent's session journal + pinned findings, which
  a human doesn't author. The human's own authoring surfaces already exist at parity (Conclusion
  panel `.md`/`.pdf` export, Notes drawer), so no separate human "compose report" UI ships here;
  the human fully *consumes* the doc (open/read/share). Re-running with the same name overwrites,
  so the agent owns naming.
