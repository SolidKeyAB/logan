...
- **Build 2 — trustable steps. ✅ BUILT 2026-08-24.** Each step shows its RESULT (hit counts / what it found), not just the verb — already half-there via the per-step `summary`; enrich it.
  > Shipped: the api-server now captures a compact **result summary** per recorded call (`summarizeReplay` — enriched to cover search/filter/analyze/time-gaps/trend/investigate-component/fold-regions/evidence-pack/crashes) onto its `JournalEntry.result` at `sendJson` time (keyed by the response object; covers success + error, skips replays). `buildTemplate` persists it on each `TemplateStep.result`, so saved investigations carry the outcome from when they were recorded; `investigationToGraph` projects it onto `WorkflowNode.result`; the ⋔ step-list renders a `→ 42 matches` line under each node. Now the graph reads as OUTCOMES, not bare verbs (Fable #3). +1 golden test.
...
**Phase 3 — Tweak + fork. ✅ BUILT 2026-08-24.**
...
> **Build 1 (tweak-then-replay) was already shipped** — `renderReplayParamForm` gives an inline noun-tweak form (window/range first) that replays with overrides via `runInvestigation(name, params, force)`, closing the old human-parity gap. Phase 3 adds the **fork**: a **⑂ Save as new…** button on that form persists the tweaked values as a NEW saved investigation (new captured defaults) via `/api/investigation-fork` (`resolveSteps` + `buildTemplate` re-extracts the param schema from the overridden bodies; requirements carry over) + `IPC.INVESTIGATION_FORK` + MCP **`logan_fork_investigation`** (agent parity). The read-only ⋔ overlay gained a **✎ Tweak & replay** button that opens that form. Net: a past hunt can be retargeted (time window / component) and kept as a reusable instance — the killer replay job — by both operators. Shared `collectReplayOverrides` feeds both Run and Fork.
