...
- **A — the entity.** New `sequence` kind + store: an ordered `SequenceClue[]` where each clue = `{ ref: 'line'|'range'|'signalValue'|'searchHit'|'finding', line?, endLine?, at?, field?, value?, note? }`, saved with name + description + scope. Pure model, surfaced via `logan_entities` / `/api/entities` + a "Sequences" group in the Saved panel. No gesture yet.
- **B — the collect gesture (both operators).** Human: right-click a selection (line / range / signal value / search hit) → **"Add to sequence"** → a live **clue tray** (light strip) + **"Save sequence as…"**. Agent: `logan_add_clue` (append to the active sequence) + save — parity with `logan_report_finding`. This is the "natural flow to collect + save."
