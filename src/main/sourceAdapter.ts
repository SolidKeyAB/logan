 ...
 *
 * The actual parser lives in mf4Parse.ts and runs inside a WORKER THREAD
 * (mf4Worker.ts) so the CPU-heavy decode never blocks the Electron main/UI event
 * loop. normalize() just spawns the worker and relays its progress. Each record
 * becomes a synthetic line `t=<master> <name>=<value> …` so the existing viewer +
 * Trends field-discovery chart the signals with zero new UI.
 */
export class Mf4Adapter implements SourceAdapter {
  ...
  async normalize(
    ...
    // Parse in a worker thread so the heavy decode never blocks the main/UI loop.
    await new Promise<void>((resolve, reject) => {
      const worker = new Worker(path.join(__dirname, 'mf4Worker.js'), {
      ...
      worker.on('message', (msg: { type: string; percent?: number; message?: string }) => {
        ...
        else if (msg?.type === 'error') finish(new Error(msg.message || 'mf4 worker error'));
      });
      worker.on('error', (err) => finish(err instanceof Error ? err : new Error(String(err))));
      worker.on('exit', (code) => { if (code !== 0) finish(new Error(`mf4 worker exited with code ${code}`)); });
    });
