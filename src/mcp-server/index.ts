  ...
  'Save the investigative steps recorded so far as a NAMED, re-runnable template (an "investigate pattern"). Values like component/field/pattern/event become fill-in parameters so the template can be replayed on a different log. Optionally attach `requirements` — the preconditions the log must meet for this investigation to apply (e.g. the file must match a saved column template / format), plus the saved entities (searches, filters, highlights, column layouts…) it expects. On replay, LOGAN preflights these and blocks a mismatched log. Call this after investigating a ticket when the user wants to reuse the approach.',
  {
    ...
    requirements: z.object({
      ...
      entities: z.array(z.object({
        ...
        id: z.string().optional(),
        name: z.string().optional(),
        autoApply: z.boolean().optional().describe('The "outfit": if this is a lens (filter/highlight/columnLayout/session), auto-apply it to the view when the recipe runs — so the log is dressed in its saved lenses before the steps execute.'),
        note: z.string().optional(),
...
server.tool(
  ...
  'Export LOGAN\'s reusable GLOBAL catalogue (searches, sessions, single-sessions, filter presets, highlight groups, bookmark sets, column layouts, column patterns, constants, trend properties, saved patterns, context definitions, clue sequences, investigations, baselines) into ONE portable `.logan-pack` file — the same stores the human ⤓ Export button packs. This is the "carry my LOGAN setup with me" file: a readable JSON container + a manifest with per-store SHA-256 checksums. It does NOT include per-log analysis (annotations/notes/findings) or secret stores (agent config, ssh profiles, connections). Pass `path` to write the file (recommended), or omit it to get the pack text back inline. Optionally restrict to certain `kinds`, or set a `passphrase` to AES-256-GCM encrypt the pack for sharing outside the team.',
  {
