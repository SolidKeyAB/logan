...
const SAMPLE = Buffer.concat([
  ...
  msg({ tsNs: 296_000_000_000, level: 2, channel: CH, source: SRC, text: '[4532:4532:1310123] [renderer]: Using simple cache' }),
  msg({ tsNs: 296_004_000_000, level: 1, channel: CH, source: SRC, text: '[4532:5990:1310123] [worker]: location callback' }),
...
describe('VtraceAdapter', () => {
  ...
  it('decodes message text, level and nanosecond timestamp', () => {
    ...
    expect(recs[0].message).toBe('[4532:4532:1310123] [renderer]: Using simple cache');
  ...
  it('normalize() writes the official 11-column export (banner + header + rows)', async () => {
    ...
    try {
      ...
      const row = lines.find(l => l.includes('[renderer]: Using simple cache'))!;
