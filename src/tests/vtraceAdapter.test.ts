...
const SAMPLE = buildFixture([
  record(296_000_000_000, 2, '[4532:4532:1310123] [renderer]: Using simple cache'),
  record(296_004_000_000, 1, '[4532:5990:1310123] [worker]: location callback'),
...
describe('VtraceAdapter', () => {
  ...
  it('decodes message text, level and nanosecond timestamp', () => {
    ...
    expect(recs[0]).toMatchObject({ tsNs: 296_000_000_000, level: 2, text: '[4532:4532:1310123] [renderer]: Using simple cache' });
  ...
  it('normalize() writes "<seconds> <LEVEL> <message>" lines, 1:1 with records', async () => {
    ...
    try {
      ...
      expect(lines[0]).toBe('296.000000 WARNING [4532:4532:1310123] [renderer]: Using simple cache');
